...
#         AGE: Edad (years).
#       PAY_0: Historia de 7pagos pasados. Estado del pago en septiembre, 2005.
...
def build_pipeline(x_train, k_features=10):
...
def optimize_hyperparameters(pipeline, x_train, y_train):
    ...
    # "sag",
    # "saga"],
    #     "classifier__class_weight": [{0: 1, 1: 1.2}, {0: 1, 1: 1.3}, {0: 1, 1: 1.4}],
    ...
    scorers = {
        'balanced_accuracy': make_scorer(precision_score),
    ...
    grid_search = GridSearchCV(
        ...
        cv=10,
        scoring=scorers,
        refit='balanced_accuracy',  # Selecciona el mejor modelo usando balanced_accuracy
        n_jobs=-1,
        verbose=4,
...
def calcular_metricas(modelo, X_train, y_train, X_test, y_test, ruta_salida='files/output/metrics.json', threshold=0.5):
    # Predecir con threshold
    y_pred_train = (modelo.predict_proba(X_train)[:, 1] > threshold).astype(int)
    y_pred_test = (modelo.predict_proba(X_test)[:, 1] > threshold).astype(int)
...
def agregar_matrices_confusion(modelo, X_train, y_train, X_test, y_test, ruta_salida='files/output/metrics.json', threshold=0.5):
    # Predecir con threshold
    y_pred_train = (modelo.predict_proba(X_train)[:, 1] > threshold).astype(int)
    y_pred_test = (modelo.predict_proba(X_test)[:, 1] > threshold).astype(int)
...
save_filtered_models(gridsearch, X_train, y_train)
calcular_metricas(gridsearch, X_train, y_train, X_test, y_test, threshold=0.5)

agregar_matrices_confusion(gridsearch, X_train, y_train, X_test, y_test, threshold=0.5)